<script lang="ts">


</script>

<div>
    <section id="features" class="bg-surface-100-800-token features-gradient svelte-1fg2m4z">
        <div class="section-container svelte-1fg2m4z">
            <div class="space-y-4 md:space-y-10">
                <div class="space-y-4 text-center" data-svelte-h="svelte-1jpjkss">
                    <h2 class="h2">My Product</h2>
                    <p>今までに制作したプロダクトの一覧ページです。</p>
                </div>
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">

                    <div class="card  card-hover variant-glass p-4 shadow-lg md:p-10 space-y-4 text-center"><span
                            class="btn-icon btn-icon-xl variant-soft-primary"><i
                                class="fa-solid fa-laptop-code"></i></span>
                        <h3 class="h3">CoboLab.dev</h3>
                        <p>自分の経歴やプロダクトをまとめるポートフォリオを作成しました</p>
                    </div>

                    <div class="card card-hover variant-glass p-4 shadow-lg md:p-10 space-y-4 text-center"><span
                            class="btn-icon btn-icon-xl variant-soft-primary"><i
                                class="fa-solid fa-laptop-code"></i></span>
                        <h3 class="h3">プロダクトのタイトル</h3>
                        <p>プロダクトの説明</p>
                    </div>
                    <div class="card card-hover variant-glass p-4 shadow-lg md:p-10 space-y-4 text-center"><span
                            class="btn-icon btn-icon-xl variant-soft-primary"><i
                                class="fa-solid fa-laptop-code"></i></span>
                        <h3 class="h3">プロダクトのタイトル</h3>
                        <p>プロダクトの説明</p>
                    </div>
                    <div class="card card-hover variant-glass p-4 shadow-lg md:p-10 space-y-4 text-center"><span
                            class="btn-icon btn-icon-xl variant-soft-primary"><i
                                class="fa-solid fa-laptop-code"></i></span>
                        <h3 class="h3">プロダクトのタイトル</h3>
                        <p>プロダクトの説明</p>
                    </div>

                    <div class="card card-hover variant-glass p-4 shadow-lg md:p-10 space-y-4 text-center"><span
                            class="btn-icon btn-icon-xl variant-soft-primary"><i
                                class="fa-solid fa-laptop-code"></i></span>
                        <h3 class="h3">プロダクトのタイトル</h3>
                        <p>プロダクトの説明</p>
                    </div>
                    <div class="card card-hover variant-glass p-4 shadow-lg md:p-10 space-y-4 text-center"><span
                            class="btn-icon btn-icon-xl variant-soft-primary"><i
                                class="fa-solid fa-laptop-code"></i></span>
                        <h3 class="h3">プロダクトのタイトル</h3>
                        <p>プロダクトの説明</p>
                    </div>

                    <div class="card card-hover variant-glass p-4 shadow-lg md:p-10 space-y-4 text-center"><span
                        class="btn-icon btn-icon-xl variant-soft-primary"><i
                            class="fa-solid fa-laptop-code"></i></span>
                            <h3 class="h3">プロダクトのタイトル</h3>
                    <p>プロダクトの説明</p>
                    </div>

                    <div class="card card-hover variant-glass p-4 shadow-lg md:p-10 space-y-4 text-center"><span
                        class="btn-icon btn-icon-xl variant-soft-primary"><i
                            class="fa-solid fa-laptop-code"></i></span>
                            <h3 class="h3">プロダクトのタイトル</h3>
                    <p>プロダクトの説明</p>
                    </div>

                    <div class="card card-hover variant-glass p-4 shadow-lg md:p-10 space-y-4 text-center"><span
                        class="btn-icon btn-icon-xl variant-soft-primary"><i
                            class="fa-solid fa-laptop-code"></i></span>
                            <h3 class="h3">プロダクトのタイトル</h3>
                    <p>プロダクトの説明</p>
                    </div>

                </div>
            </div>
        </div>
    </section>
</div>



<style lang="postcss">
</style>